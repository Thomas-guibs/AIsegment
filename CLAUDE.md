...
- **MRR sous gestion** (`lib/analytics/portfolio.ts`): per company at T (1st of month, UTC) = Σ signed `amount` of **won** deals (Closed Won `closedlost` + Paiement reçu `143474109`) with effective date (`date_de_paiement` → `date_de_prise_en_compte` → `closedate`) < T. Counted only if a CSM is known at T, `phase_du_client` at T ∈ `MRR_PHASES` (Onboarding / Activated / Run / Parent company), **and at least one associated deal has a `date_de_paiement` < T** (the company-level `date_de_paiement` is unreliable — deals are the source). Won-but-unpaid deals of a paying customer count (contracted MRR) and are surfaced in diagnostics. Never use `total_revenue` — it ignores downsells. CSM and phase are read point-in-time from HubSpot property history (`lib/hubspot/history.ts`, backfilled to `hs_createdate`).
