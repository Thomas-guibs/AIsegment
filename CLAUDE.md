...
- **MRR sous gestion** (`lib/analytics/portfolio.ts`): per company at T (1st of month, UTC) = Σ signed `amount` of deals in stage « Paiement reçu » (`SALES_STAGES.PAIEMENT_RECU`) with effective date (`date_de_paiement` → `date_de_prise_en_compte` → `closedate`) < T. Counted only if a CSM is known at T and `lifecyclestage` at T = `customer` (label « Client », `CUSTOMER_LIFECYCLE`). Never use `total_revenue` — it ignores downsells. CSM and lifecycle are read point-in-time from HubSpot property history (`lib/hubspot/history.ts`, backfilled to `hs_createdate`).
