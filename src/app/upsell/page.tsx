    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1 bg-card rounded-lg p-0.5 border border-card-border">
          {([["analyses", "Analyses"], ["liste", "Liste"]] as const).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={cn(
      ...
      {loading || !data ? (
        ...
        <ListeTab data={data} csmFilter={csmFilter} setCsmFilter={setCsmFilter} />
      )}
    </div>
  )
}

function AnalysesTab({ data }: { data: UpsellAnalyticsResponse }) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card">
          ...
          <BarChartComponent data={data.byMonthCsm} series={csmSeries} xKey="monthLabel" stacked height={280} />
        </div>
        <div className="card">
          <h3 className="text-sm font-medium text-text-secondary mb-4">Upsell par mois — Total</h3>
          <LineChartComponent data={data.byMonthTotal} series={[{ key: "Upsell", label: "Upsell", color: "#22C55E" }]} xKey="monthLabel" height={280} formatValue={(v) => formatCurrency(v, true)} />
        </div>
        <div className="card">
          <h3 className="text-sm font-medium text-text-secondary mb-4">Upsell par trimestre</h3>
          <BarChartComponent data={data.byQuarter} series={[{ key: "Upsell", label: "Upsell", color: "#2563EB" }]} xKey="quarterLabel" height={280} />
        </div>
        <div className="card">
          <h3 className="text-sm font-medium text-text-secondary mb-4">Panier moyen des transactions d'upsell</h3>
          <LineChartComponent data={data.avgByMonth} series={[{ key: "Moyenne", label: "Panier moyen", color: "#8B5CF6" }]} xKey="monthLabel" height={280} formatValue={(v) => formatCurrency(v, true)} />
        </div>
        ...
        <div className="card">
          ...
          <BarChartComponent
            data={data.pipelineByStage.map((s) => ({ stage: s.label, Montant: s.amount }))}
...
function ListeTab({
  ...
  data: UpsellAnalyticsResponse
  csmFilter: string
  setCsmFilter: (v: string) => void
}) {
  const csmNames = useMemo(() => {
    const unique = new Set(data.dealsList.map((d) => d.csmName))
    return Array.from(unique).sort()
  }, [data.dealsList])

  const filtered = useMemo(() => {
    if (csmFilter === "all") return data.dealsList
    return data.dealsList.filter((d) => d.csmName === csmFilter)
  }, [data.dealsList, csmFilter])
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-text-muted">CSM :</span>
        <button
        ...
        {csmNames.map((name) => (
          <button
            key={name}
            onClick={() => setCsmFilter(csmFilter === name ? "all" : name)}
            className={cn(
              "px-2.5 py-1 text-[11px] rounded-md transition-colors font-medium",
              csmFilter === name
                ? "bg-accent text-white"
        ...
        <span className="text-xs text-text-muted ml-auto">
          {filtered.length} transactions — {formatCurrency(totalFiltered, true)}
        </span>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-background/50">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs text-text-muted font-medium">Transaction</th>
                <th className="px-4 py-2.5 text-left text-xs text-text-muted font-medium">Company</th>
                <th className="px-4 py-2.5 text-left text-xs text-text-muted font-medium">CSM</th>
                <th className="px-4 py-2.5 text-right text-xs text-text-muted font-medium">Montant</th>
                <th className="px-4 py-2.5 text-left text-xs text-text-muted font-medium">Operation Date</th>
                <th className="px-4 py-2.5 text-left text-xs text-text-muted font-medium">Payment Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-card-border">
              {filtered.length === 0 ? (
                ...
                filtered.map((deal) => (
                  <tr key={deal.id} className="hover:bg-card-hover transition-colors">
                    <td className="px-4 py-2.5 text-sm text-text-primary font-medium max-w-[240px] truncate">{deal.name}</td>
                    <td className="px-4 py-2.5 text-sm text-text-secondary max-w-[180px] truncate">{deal.companyName}</td>
                    <td className="px-4 py-2.5 text-sm text-text-secondary">{deal.csmName}</td>
                    <td className="px-4 py-2.5 text-sm font-mono text-positive text-right">{formatCurrency(deal.amount, true)}</td>
                    <td className="px-4 py-2.5 text-sm text-text-secondary">{deal.operationDate ? formatDateFR(deal.operationDate) : "—"}</td>
                    <td className="px-4 py-2.5 text-sm text-text-secondary">{deal.paymentDate ? formatDateFR(deal.paymentDate) : "—"}</td>
                  </tr>
                ))
              )}
            </tbody>
